 ...
 * @example formatAddress('0x1234567890abcdef') => '0x1234...cdef'
 */
export function formatAddress(address: string | undefined, length = CONSTANTS.ADDRESS_DISPLAY_LENGTH): string {
  if (!address) return '';
  const prefixLength = Math.floor(length / 2);
  const suffixLength = Math.ceil(length / 2) - 2; // -2 for '0x'
  return `${address.slice(0, prefixLength)}...${address.slice(-suffixLength)}`;
 ...
 * Format currency (ETH/USDT)
 */
export function formatCurrency(value: string | number, decimals = 4, currency = 'USDT'): string {
  const num = typeof value === 'string' ? parseFloat(value) : value;
  return `${num.toFixed(decimals)} ${currency}`;
}
 ...
 * Get event status based on timestamps
 */
export function getEventStatus(startTime: number, endTime: number, isPaid: boolean): string {
  const now = Date.now() / 1000;
 ...
 * Get block explorer URL for address/tx
 */
export function getBlockExplorerUrl(value: string, type: 'address' | 'tx'): string {
  const baseUrl = import.meta.env.VITE_BLOCK_EXPLORER;
  return type === 'address' ? `${baseUrl}/address/${value}` : `${baseUrl}/tx/${value}`;
}
